		...
		<BR>
			<div class="panel panel-default">
			  ...
			  <div class="panel-body">
			  	<form action="{{ route('formulir.update' , $formulirs->id) }}" method="post" >
			  		{{ csrf_field() }}
			  		<input name="_method" type="hidden" value="PATCH">
			  <div class="form-group {{ $errors->has('nomer_formulir') ? ' has-error' : '' }}">
			  			<label class="control-label">Nomer Formulir</label>	
			  			<input type="number" name="nomer_formulir" class="form-control" value="{{$formulirs-> nomer_formulir}}" required>
			  		...
			  		<div class="form-group {{ $errors->has('biaya') ? ' has-error' : '' }}">
			  			...
			  			<input type= "number" name="biaya" class="form-control" rows="10" value="{{$formulirs->biaya }}"  required>
			  			@if ($errors->has('biaya'))
                            <span class="help-block">
			  		...
			  		<div class="form-group {{ $errors->has('id_anggota') ? ' has-error' : '' }}">
			  			...
			  			<select name="id_anggota" class="form-control">
			  				...
			  				<option value="{{ $data->id }}" value="{{ $data->id }}"  {{ $selectedanggota == $data->id ? 'selected="selected"' : '' }} > {{ $data->nama }}</option>
			  				@endforeach
			  			</select>
